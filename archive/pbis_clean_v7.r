...
# Create sum of EBS scores
  ...
  b <- transform(b,
                    perc.in.place.sw.disc.sys  = sw.disc.sys  / r1[2],
                    perc.in.place.ncr.set      = ncr.set      / r2[2],
                    perc.in.place.cr.set       = cr.set       / r3[2],
                    perc.in.place.ind.stud.sys = ind.stud.sys / r4[2])
...
lm.data$i.set <- ifelse(!is.na(lm.data$set_implementationaverage), 1, 0)
